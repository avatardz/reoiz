...
(function() {
    ...
    this.getPositionOffset = function() {
        try {
            var properties = Entities.getEntityProperties(this.entityID, ["userData"]);
            var userData = JSON.parse(properties.userData);

            // If MyAvatar return my uuid
            if (userData.seat.offset !== undefined) {
                return userData.seat.offset;
            }
        } catch (e) {
            // Do nothing
        }
    ...
    this.checkSeatForAvatar = function() {
        // not seated when locked
        if (Entities.getEntityProperties(this.entityID, 'locked').locked) {
            return false;
        }
    ...
    this.sitDown = function() {
        ...
        var index = MyAvatar.getJointIndex(PIN_JOINT_NAME);

        MyAvatar.pinJoint(index, Vec3.sum(properties.position, this.getPositionOffset()), properties.rotation);
    ...
    this.standUp = function() {
        ...
        if (!Entities.getEntityProperties(this.entityID, 'locked').locked && MyAvatar.sessionUUID === this.getSeatUser()) {
